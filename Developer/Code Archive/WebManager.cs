...
public class WebManager : MonoBehaviour
{
	...
	Uri url;
	string[] allSongs;
	String[] featuredSongs;

	static string path;
	public bool showFeaturedDlL = false;
	...
	int file;
	List<Song> featuredSongsList = new List<Song>();
	...
	void Start ()
	{
		...
		try
		{
			...
			featuredSongs = wClient.DownloadString ("http://raw.github.com/2CatStudios/UnityMusicPlayer/master/Developer/FeaturedSongs.txt").Split ('\n');
...
*/
	...
	void OnGUI ()
	{
		...
		if (showFeaturedDlL == true)
			GUI.Window (1, new Rect(300, 200, 200, 300), FeaturedDownloads, "Featured Downloads");
		...
		if (showAllDlL == true)
			GUI.Window (6, new Rect (Screen.width / 2 - 300, Screen.height / 2 - 250, 600, 500), AllDownloads, "OnlineMusicBrowser");
	...
	void DownloadInfo (int wid)
	{
		...
		GUILayout.Label ("Name: " + featuredSongsList[file].name);
		GUILayout.Label ("Artist: " + featuredSongsList[file].artist);
		GUILayout.Label ("Album: " + featuredSongsList[file].album);
		GUILayout.Label ("Genre: " + featuredSongsList[file].genre);
		GUILayout.Label ("Format: " + featuredSongsList[file].format);
		if (GUILayout.Button ("Support Artist"))
		{

			Process.Start (featuredSongsList[file].supportLink);
		}
	...
	void AllDownloads(int wid)
	{
		...
		if (GUILayout.Button ("All"))
		{
			...
			UnityEngine.Debug.Log ("All");
		}

		GUILayout.BeginArea(new Rect(200, 100, 600, 500));
	...
	void Download ()
	{
		...
		try
		{
			...
			client.DownloadFileAsync (url, path + Path.DirectorySeparatorChar + "Media" + Path.DirectorySeparatorChar + featuredSongsList[file].name + "." + featuredSongsList[file].format);
		} catch (Exception error)
		{
